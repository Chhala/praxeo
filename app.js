...
const NOTES_KEY       = 'praxeo_notes_v1';
...
function saveNotes() {
  try { localStorage.setItem(NOTES_KEY, JSON.stringify(state.notes)); } catch(e) { console.error('[Praxeo] saveNotes échoué:', e); }
}
...
function loadNotes() {
  ...
  } catch(e) { console.error('[Praxeo] loadNotes échoué:', e); }
}
...
function loadState() {
  ...
  const snapV2  = parseSnap(localStorage.getItem(STORAGE_KEY));
  const snapV1  = parseSnap(localStorage.getItem(STORAGE_KEY_OLD));
  const snap    = snapV2 || snapV1;
  const migrating = !snapV2 && !!snapV1;
  ...
  if (snap.praxis && snap.praxis.length) {
    ...
    if (migrating) saveState(); // persiste en v2 seulement si migration
    return true;
  }
  ...
  el.querySelectorAll('[data-type]').forEach(b => {
    ...
    b.addEventListener('click', e => {
      ...
      if (type === 'routine') {
        ...
        explodeBubble(b, () => { state.notes.splice(nidx, 1); saveNotes(); saveState(); renderAccueil(); });
      }
    });
  });
...
function importData(data) {
  ...
  if (data.notes)                 { state.notes = data.notes; saveNotes(); }
