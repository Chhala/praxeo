...
const PAGE_SUBTITLES = { accueil: '', praxis: '', stats: '' };
...
function initSwipeNav() {
  ...
  function isSwipeZone(e) {
    ...
    if (page === 'stats') {
      ...
      return navbar ? e.clientY > navbar.getBoundingClientRect().top - 480 : true;
    }
    if (page === 'accueil') {
      const row = document.getElementById('rowNote');
      return row && row.getBoundingClientRect().bottom !== 0 ? e.clientY > row.getBoundingClientRect().bottom : false;
    }
    if (page === 'praxis') {
      const enc = document.querySelector('#page-praxis .praxis-encart');
      return enc && enc.getBoundingClientRect().bottom !== 0 ? e.clientY > enc.getBoundingClientRect().bottom : false;
    }
    return false;
  }
  ...
  document.addEventListener('pointerdown', e => {
    if (_accueilDndActive || _praxisDndActive || (state.sheet && state.sheet.open) || !isSwipeZone(e)) return;
...
function doUndo() {
  ...
  else if (action.type === 'accueil_note_remove' || action.type === 'accueil_note_tap') {
...
function renderPraxis() {
  ...
  el.innerHTML = `
    ...
    <div class="encart praxis-encart">
      ...
      <div class="praxis-bubble-list" id="praxisBubbleList">
        ${list.length ? list.map(p => renderPraxisItem(p)).join('') : `<span class="list-empty">Liste vide</span>`}
      </div>
  ...
  el.querySelectorAll('.praxis-item').forEach(item => {
    ...
    let pressTimer = null;
    item.addEventListener('pointerdown', () => { pressTimer = setTimeout(() => enterWiggleMode(id), 900); });
    ['pointerup','pointercancel','pointermove'].forEach(ev => item.addEventListener(ev, () => clearTimeout(pressTimer)));
  ...
  el.addEventListener('click', e => { if (state.wiggleId && !e.target.closest('.praxis-item')) exitWiggleMode(); });
}
...
function renderPraxisItem(p) {
  ...
  const styleAttr = isLong ? `style="border-color:${p.color};color:${p.color};"` : `style="background:${p.color};"`;

  const checkSvg = p.active
    ? `<svg class="praxis-checkbox" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" fill="#4caf50"/><path d="M4.5 8 L7 10.5 L11.5 5.5" stroke="#fff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`
    : `<svg class="praxis-checkbox" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="${isLong ? p.color : 'rgba(255,255,255,0.5)'}" stroke-width="1.5" fill="none"/></svg>`;

  const gaugeFill = isLong ? `<div class="gauge-fill" style="width:${Math.min((p.progress||0)*25,100)}%;background:${p.color};"></div>` : '';
  const badges = wiggling ? `<div class="praxis-badges-group"><div class="edit-badge" data-id="${p.id}"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20l3-1L19 7a2 2 0 00-3-3L4 16l-1 4z"/><line x1="14" y1="6" x2="18" y2="10"/></svg></div></div>` : '';

  return `<div class="${cls}" ${styleAttr} data-id="${p.id}">${gaugeFill}${checkSvg}<span class="praxis-item-label">${p.label}</span>${badges}</div>`;
}
...
function enterWiggleMode(id) { state.wiggleId = id; renderPraxis(); }
function exitWiggleMode() { if (state.wiggleId) { state.wiggleId = null; if (state.currentPage === 'praxis') renderPraxis(); } }
...
function initPraxisDragDrop(el) {
  ...
  let startX = 0, startY = 0, isDragging = false, wasDragging = false, overTrash = false, activePointerId = null;
  ...
  function cleanup() {
    ...
    dragEl = null; dragId = null; isDragging = false; overTrash = false; activePointerId = null;
  }
  ...
  function onDocMove(e) {
    ...
    if (!isDragging) {
      ...
      clone.style.cssText = `position:fixed;z-index:9999;pointer-events:none;opacity:0.8;left:${r.left}px;top:${r.top}px;width:${r.width}px;margin:0;`;
  ...
  el.querySelectorAll('.praxis-item').forEach(item => {
    item.addEventListener('pointerdown', e => {
      if (_praxisDndActive || state.wiggleId || e.target.closest('.edit-badge,.delete-badge,.praxis-badges-group')) return;
      _praxisDndActive = true; dragEl = item; dragId = item.dataset.id;
      startX = e.clientX; startY = e.clientY; wasDragging = false; activePointerId = e.pointerId;
      document.addEventListener('pointermove', onDocMove, { passive: false });
      document.addEventListener('pointerup', onDocUp);
      document.addEventListener('pointercancel', cleanup);
    });
    item.addEventListener('click', e => { if (wasDragging) { e.stopImmediatePropagation(); e.preventDefault(); wasDragging = false; } }, true);
  });
}
  ...
  return `
    ...
    <div class="sheet-row">
      ...
      <div class="type-radio-group">
        ${TYPES.map(t => `<button class="type-radio ${s.type===t?'selected':''}" data-type="${t}">${TYPE_LABELS[t]}</button>`).join('')}
      </div>
    </div>
    <div class="sheet-row">
      <div class="name-preview-row">
        <input class="sheet-input sheet-input-short" id="sheetInput" type="text" placeholder="Nom…" value="${s.label}" maxlength="19" autocapitalize="characters" spellcheck="false">
        <div class="sheet-preview-inline" id="sheetPreview">${renderPreviewBubble()}</div>
      </div>
    </div>
    <div class="sheet-row">
      ...
      <div class="color-days-row">
        <div class="color-palette">
          ${COLORS.map(c => `<div class="color-swatch ${s.color===c?'selected':''}" style="background:${c};" data-color="${c}"></div>`).join('')}
        </div>
        ${s.type==='routine' ? `<div class="days-grid-inline">${DAY_NAMES.map((d,i) => `<button class="day-btn-sm ${s.days.includes(i+1)?'selected':''}" data-day="${i+1}">${d}</button>`).join('')}</div>` : ''}
      </div>
    </div>
    <div class="sheet-actions">
      <button class="btn-sheet btn-cancel" id="btnSheetCancel">Annuler</button>
      ${editMode ? `<button class="btn-sheet btn-create-action" id="btnSheetSave">Modifier</button>` : `<button class="btn-sheet btn-create-action" id="btnSheetCreate">Créer</button><button class="btn-sheet btn-activate" id="btnSheetActivate">Créer & activer</button>`}
    </div>
  `;
}

function renderPreviewBubble() {
  const s = state.sheet; if (!s.label) return '';
  const label = s.label.toUpperCase();
  if (s.type === 'long') return `<div class="bubble-long" style="border-color:${s.color};color:${s.color};"><div class="gauge-fill" style="width:0%;background:${s.color};"></div><span>${label}</span></div>`;
...
function bindSheetEvents(sheet, editMode) {
  sheet.addEventListener('click', e => {
    const typeBtn = e.target.closest('.type-radio'); if (typeBtn) { state.sheet.type = typeBtn.dataset.type; refreshSheet(sheet, editMode); return; }
    const swatch = e.target.closest('.color-swatch'); if (swatch)  { state.sheet.color = swatch.dataset.color; refreshSheet(sheet, editMode); return; }
    const dayBtn = e.target.closest('.day-btn-sm');
    if (dayBtn) {
      const d = parseInt(dayBtn.dataset.day); const idx = state.sheet.days.indexOf(d);
      if (idx > -1) state.sheet.days.splice(idx, 1); else state.sheet.days.push(d);
      refreshSheet(sheet, editMode); return;
    }
  ...
  sheet.addEventListener('input', e => {
    if (e.target.id === 'sheetInput') {
      const input = e.target, pos = input.selectionStart; const upper = input.value.toUpperCase().slice(0, 19);
      input.value = upper; input.setSelectionRange(pos, pos); state.sheet.label = upper; updatePreview(sheet);
    }
  });
}
...
function refreshSheet(sheet, editMode) {
  ...
  const fresh = getSheet(); fresh.innerHTML = buildSheetHTML(editMode); fresh.classList.add('open');
...
function openSheet() {
  state.sheet = { open:true, type:'tache', label:'', color: COLORS[Math.floor(Math.random() * COLORS.length)], days:[1,2,3,4,5] };
  const sheet = getSheet(); sheet.innerHTML = buildSheetHTML(false); bindSheetEvents(sheet, false);
...
function openEditSheet(id) {
  ...
  state.editId = id; state.sheet = { open:true, type:p.type, label:p.label, color:p.color, days:p.days ? [...p.days] : [1,2,3,4,5] };
  exitWiggleMode(); const sheet = getSheet(); sheet.innerHTML = buildSheetHTML(true); bindSheetEvents(sheet, true);
  document.getElementById('sheetOverlay').classList.add('open'); sheet.classList.add('open');
  setTimeout(() => sheet.querySelector('#sheetInput')?.focus(), 350);
}

function openConfirmDeleteSheet(id, label) {
  const overlay = document.getElementById('sheetOverlay'); const sheet = getSheet();
  sheet.innerHTML = `<div class="sheet-handle"></div><div class="sheet-confirm-text">Supprimer <strong>${label}</strong> ?</div><div class="sheet-confirm-sub">Cette action est irréversible.</div><div class="sheet-actions" style="margin-top:20px;"><button class="btn-sheet btn-cancel" id="btnConfirmCancel">Annuler</button><button class="btn-sheet btn-delete" id="btnConfirmDelete">Supprimer</button></div>`;
...
function createPraxis(activate) {
  ...
  state.praxis.push({ id: 'p_' + Date.now(), type: state.sheet.type, label, color: state.sheet.color, active: activate, ...(state.sheet.type === 'routine' ? { days: [...state.sheet.days] } : {}), ...(state.sheet.type === 'long' ? { progress: 0 } : {}) });
  saveState(); closeSheet(); renderPraxis();
}

function savePraxis() {
  const label = state.sheet.label.trim(); if (!label || !state.editId) return;
  const p = state.praxis.find(x => x.id === state.editId); if (!p) return;
  p.label = label; p.color = state.sheet.color; p.type = state.sheet.type; if (state.sheet.type === 'routine') p.days = [...state.sheet.days];
  saveState(); closeSheet(); renderPraxis();
}
...
function saveAccueil() {
  try {
    const snap = { routinesDone: accueil.routinesDone, routinesSkipped: accueil.routinesSkipped, tachesDone: accueil.tachesDone, tachesRemoved: accueil.tachesRemoved, longsRemoved: accueil.longsRemoved };
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && k.startsWith('praxeo_accueil_') && !k.endsWith(today)) {
        const d = k.replace('praxeo_accueil_', ''); if ((new Date(today) - new Date(d)) / 86400000 > 7) { localStorage.removeItem(k); i--; }
      }
    }
  } catch(e) {}
}

const accueil = { routinesDone: {}, routinesSkipped: {}, tachesDone: {}, tachesRemoved: {}, longsRemoved: {}, wiggleId: null, wiggleType: null, gaugeEditId: null };
...
function recalibrateTodayStats() {
  ...
  const key = todayKey(); const entry = state.statsHistory[key]; if (!entry) return;
  const dow = todayDow(); const allActive = state.praxis.filter(p => p.type === 'routine' && p.active && (!p.days || p.days.includes(dow)));
  const doneIds = Object.keys(accueil.routinesDone); const done = doneIds.filter(id => allActive.some(p => p.id === id)).length; const total = allActive.length;
  if (entry.done !== done || entry.total !== total) { state.statsHistory[key] = { done, total, ids: doneIds }; saveState(); }
}

function recordRoutineDone() {
  const key = todayKey(); const dow = todayDow(); const allActive = state.praxis.filter(p => p.type === 'routine' && p.active && (!p.days || p.days.includes(dow)));
  const doneIds = Object.keys(accueil.routinesDone); const done = doneIds.filter(id => allActive.some(p => p.id === id)).length; const total = allActive.length;
  if (!state.statsHistory) state.statsHistory = {}; state.statsHistory[key] = { done, total, ids: doneIds };
  const streak = computeStreak(state.statsHistory); if (streak > (state.statsRecord || 0)) state.statsRecord = streak;
  saveState();
}
...
function renderAccueil() {
  const el = document.getElementById('page-accueil'); const dow = todayDow();
  const routines = state.praxis.filter(p => p.type==='routine' && p.active && (!p.days || p.days.includes(dow)) && !accueil.routinesSkipped[p.id] && !accueil.routinesDone[p.id]);
  const taches = state.praxis.filter(p => p.type==='tache' && p.active && !accueil.tachesRemoved[p.id]);
  const longs = state.praxis.filter(p => p.type==='long' && p.active && !accueil.longsRemoved[p.id]);

  el.innerHTML = `
    ${state.frozen ? `<div class="frozen-banner">⏸ Application gelée — bloc-notes actif</div>` : ''}
    <div class="encart-section fade-in"><div class="section-label">Routines</div><div class="encart"><div class="bubble-row" id="rowRoutine" data-section="routine">${routines.map(p => renderAccueilBubble(p,'routine')).join('')}</div><div class="btn-add-row"><button class="btn-add" data-section="routine">+</button></div></div></div>
    <div class="encart-section fade-in" style="animation-delay:.05s"><div class="section-label">Tâches du jour</div><div class="encart"><div class="bubble-row" id="rowTache" data-section="tache">${taches.map(p => renderAccueilBubble(p,'tache')).join('')}</div><div class="btn-add-row"><button class="btn-add" data-section="tache">+</button></div></div></div>
    <div class="encart-section fade-in" style="animation-delay:.1s"><div class="section-label">Long terme</div><div class="encart"><div class="bubble-row" id="rowLong" data-section="long">${longs.map(p => renderAccueilBubble(p,'long')).join('')}</div><div class="btn-add-row"><button class="btn-add" data-section="long">+</button></div></div></div>
    <div class="encart-section fade-in" style="animation-delay:.15s"><div class="section-label">Bloc-notes</div><div class="encart"><div class="bubble-row" id="rowNote" data-section="note">${state.notes.map((n,i) => renderNoteBubble(n,i)).join('')}</div><div class="btn-add-row"><button class="btn-add" data-section="note">+</button></div></div></div>
  `;
  bindAccueilEvents(el); initAccueilDragDrop(el);
}

function renderAccueilBubble(p, type) {
  const wiggling = accueil.wiggleId === p.id && accueil.wiggleType === type; const gaugeEdit = accueil.gaugeEditId === p.id;
  const delBadge = wiggling ? `<div class="acc-badge acc-badge-delete" data-id="${p.id}" data-type="${type}">−</div>` : '';

  if (type === 'routine') return `<div class="bubble bubble-routine${wiggling?' acc-wiggle':''}" style="background:${p.color}; position:relative;" data-id="${p.id}" data-type="routine" >${p.label}${delBadge}</div>`;
  if (type === 'tache') { const done = accueil.tachesDone[p.id]; return `<div class="bubble${done?' bubble-done':''}${wiggling?' acc-wiggle':''}" style="background:${p.color};${done?'opacity:.38;':''} position:relative;" data-id="${p.id}" data-type="tache" >${p.label}${delBadge}</div>`; }
  if (type === 'long') {
    const badges = wiggling ? `<div class="acc-badges-group"><div class="acc-badge acc-badge-edit" data-id="${p.id}"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20l3-1L19 7a2 2 0 00-3-3L4 16l-1 4z"/><line x1="14" y1="6" x2="18" y2="10"/></svg></div></div>` : '';
    return `<div class="acc-bubble-wrap${wiggling?' acc-wiggle':''}${gaugeEdit?' gauge-editing-wrap':''}" data-id="${p.id}" data-type="long" ><div class="bubble-long${gaugeEdit?' gauge-editing':''}" style="border-color:${p.color};color:${p.color};"><div class="gauge-fill" style="width:${Math.min((p.progress||0)*25,100)}%;background:${p.color};"></div><span>${p.label}</span></div>${badges}</div>`;
  }
  return '';
}

function renderNoteBubble(note, idx) {
  const wiggling = accueil.wiggleId === 'note_'+idx && accueil.wiggleType === 'note'; const badge = wiggling ? `<div class="acc-badge acc-badge-delete" data-noteidx="${idx}" data-type="note">−</div>` : '';
  const noteStyle = note.color ? `position:relative;background:${note.color};color:#fff;border-color:${note.color};` : `position:relative;`;
  return `<div class="bubble-note-style${wiggling?' acc-wiggle':''}" style="${noteStyle}" data-noteidx="${idx}" data-type="note">${note.label.toLowerCase()}${badge}</div>`;
}

function bindAccueilEvents(el) {
  el.addEventListener('click', e => {
    if (accueil.gaugeEditId && !e.target.closest('[data-type="long"]')) { accueil.gaugeEditId = null; renderAccueil(); return; }
    if (accueil.wiggleId && !e.target.closest('[data-id],[data-noteidx]')) exitAccueilWiggle();
  });

  el.querySelectorAll('[data-type]').forEach(b => {
    if (b.classList.contains('btn-add') || b.classList.contains('acc-badge-delete') || b.classList.contains('acc-badge-edit') || b.classList.contains('acc-badges-group')) return;
    const type = b.dataset.type, id = b.dataset.id, nidx = b.dataset.noteidx !== undefined ? parseInt(b.dataset.noteidx) : null;
    let pressTimer = null;
    ...
    ['pointerup','pointercancel','pointermove'].forEach(ev => b.addEventListener(ev, () => clearTimeout(pressTimer)));

    b.addEventListener('click', e => {
      if (e.target.closest('.acc-badge-delete') || e.target.closest('.acc-badge-edit') || e.target.closest('.acc-badges-group') || accueil.wiggleId) return;
      if (state.frozen && type !== 'note') return;

      if (type === 'routine') {
        pushUndo({ type:'accueil_routine', id, page:'accueil' }); accueil.routinesDone[id] = true; recordRoutineDone(); saveAccueil(); explodeBubble(b, () => renderAccueil());
      }
      else if (type === 'tache') {
        if (accueil.tachesDone[id]) {
          ...
          explodeBubble(b, () => {
            ...
            const p = state.praxis.find(x => x.id === id); if (p) { p.active = false; saveState(); }
            saveAccueil(); renderAccueil();
          });
        } else {
          pushUndo({ type:'accueil_tache_done', id, page:'accueil' }); accueil.tachesDone[id] = true; saveAccueil(); b.style.opacity = '0.38';
        }
      }
      else if (type === 'long') {
        const p = state.praxis.find(x => x.id === id); if (!p) return;
        const prevProgress = p.progress || 0; p.progress = prevProgress + 1;
        if (p.progress >= 4) {
          pushUndo({ type:'accueil_long_progress', id, prevProgress, exploded: true, page:'accueil' }); saveState();
          explodeBubble(b.querySelector('.bubble-long') || b, () => { p.active = false; p.progress = 0; accueil.longsRemoved[id] = true; saveAccueil(); saveState(); renderAccueil(); });
        } else {
          pushUndo({ type:'accueil_long_progress', id, prevProgress, exploded: false, page:'accueil' }); saveState();
          const fill = b.querySelector('.gauge-fill'); if (fill) fill.style.width = (p.progress * 25) + '%';
        }
      }
      else if (type === 'note') {
  ...
  el.querySelectorAll('.acc-badge-delete').forEach(badge => badge.addEventListener('click', e => { e.stopPropagation(); if (!state.frozen || badge.dataset.type === 'note') removeFromAccueil(badge.dataset.type, badge.dataset.id, badge.dataset.noteidx !== undefined ? parseInt(badge.dataset.noteidx) : null); }));
  el.querySelectorAll('.acc-badge-edit').forEach(badge => badge.addEventListener('click', e => { e.stopPropagation(); if (!state.frozen) { exitAccueilWiggle(); openGaugeSheet(badge.dataset.id); } }));
  el.querySelectorAll('.btn-add').forEach(btn => {
    let btnDown = false;
    btn.addEventListener('pointerdown', e => { e.stopPropagation(); btnDown = true; });
    btn.addEventListener('pointerup', e => { e.stopPropagation(); if (btnDown && (!state.frozen || btn.dataset.section === 'note')) { btnDown = false; exitAccueilWiggle(); if (btn.dataset.section === 'note') openNoteSheet(); else openPickerSheet(btn.dataset.section); } });
    btn.addEventListener('pointercancel', () => { btnDown = false; });
  });
}

function enterAccueilWiggle(id, type) { accueil.wiggleId = id; accueil.wiggleType = type; renderAccueil(); }
function exitAccueilWiggle() { if (accueil.wiggleId || accueil.gaugeEditId) { accueil.wiggleId = null; accueil.wiggleType = null; accueil.gaugeEditId = null; if (state.currentPage === 'accueil') renderAccueil(); } }

function removeFromAccueil(type, id, nidx) {
  if (type === 'routine') { pushUndo({ type:'accueil_skip', id, page:'accueil' }); accueil.routinesSkipped[id] = true; recordRoutineDone(); }
  else if (type === 'tache') { pushUndo({ type:'accueil_tache_remove', id, wasDone: !!accueil.tachesDone[id], page:'accueil' }); accueil.tachesRemoved[id] = true; delete accueil.tachesDone[id]; }
  else if (type === 'long') { pushUndo({ type:'accueil_long_skip', id, page:'accueil' }); accueil.longsRemoved[id] = true; }
  else if (type === 'note' && nidx !== null) { pushUndo({ type:'accueil_note_remove', note: { ...state.notes[nidx] }, nidx, page:'accueil' }); state.notes.splice(nidx, 1); saveState(); }
  accueil.wiggleId = accueil.wiggleType = null; saveAccueil(); renderAccueil();
}
...
function initAccueilDragDrop(el) {
  ['rowRoutine','rowTache','rowLong','rowNote'].forEach(rowId => {
    const row = el.querySelector('#'+rowId); if (!row) return;
    let dragEl = null, clone = null, placeholder = null, startX = 0, startY = 0, offsetX = 0, offsetY = 0, isDragging = false, wasDragging = false, activePointerId = null;
    const getSiblings = () => [...row.querySelectorAll('[data-id],[data-noteidx]')].filter(b => b !== placeholder);

    function cleanup() {
      if (clone) clone.remove(); if (placeholder) placeholder.remove(); if (dragEl) { dragEl.style.display = ''; dragEl.style.opacity = ''; }
      document.removeEventListener('pointermove', onDocMove); document.removeEventListener('pointerup', onDocUp); document.removeEventListener('pointercancel', cleanup);
      _accueilDndActive = false; dragEl = null; isDragging = false; activePointerId = null;
    }

    function onDocMove(e) {
      if (!dragEl || e.pointerId !== activePointerId) return;
      const dx = e.clientX - startX, dy = e.clientY - startY; if (!isDragging && Math.sqrt(dx*dx + dy*dy) < 12) return;
      e.preventDefault();

      if (!isDragging) {
        isDragging = true; wasDragging = true; const r = dragEl.getBoundingClientRect();
        placeholder = document.createElement('div'); placeholder.style.cssText = `display:inline-flex;width:${r.width}px;height:${r.height}px;flex-shrink:0;opacity:0;`;
        dragEl.parentNode.insertBefore(placeholder, dragEl); dragEl.style.display = 'none';
        clone = dragEl.cloneNode(true); clone.querySelectorAll('.acc-badge,.acc-badges-group').forEach(b => b.remove());
        clone.style.cssText = `position:fixed;z-index:9999;pointer-events:none;opacity:0.85;left:${r.left}px;top:${r.top}px;width:${r.width}px;margin:0;`;
        document.body.appendChild(clone);
      }
      clone.style.left = (e.clientX - offsetX) + 'px'; clone.style.top = (e.clientY - offsetY) + 'px';
      let inserted = false;
      for (const sib of getSiblings()) { const r = sib.getBoundingClientRect(); if (e.clientX < r.left + r.width / 2) { row.insertBefore(placeholder, sib); inserted = true; break; } }
      if (!inserted) row.appendChild(placeholder);
    }

    function onDocUp() { if (dragEl && isDragging && placeholder) row.insertBefore(dragEl, placeholder); cleanup(); }

    row.addEventListener('pointerdown', e => {
      if (_accueilDndActive || e.target.closest('.acc-badge,.acc-badges-group,.btn-add')) return;
      const bubble = e.target.closest('[data-id],[data-noteidx]'); if (!bubble) return;
      _accueilDndActive = true; dragEl = bubble; startX = e.clientX; startY = e.clientY;
      const r = bubble.getBoundingClientRect(); offsetX = e.clientX - r.left; offsetY = e.clientY - r.top; activePointerId = e.pointerId;
      document.addEventListener('pointermove', onDocMove, { passive: false }); document.addEventListener('pointerup', onDocUp); document.addEventListener('pointercancel', cleanup);
    });
    row.addEventListener('click', e => { if (wasDragging) { e.stopImmediatePropagation(); e.preventDefault(); wasDragging = false; } }, true);
  });
}
...
function pickerItemHTML(p) {
  if (p.type === 'long') return `<div class="picker-item bubble-long" style="border-color:${p.color};color:${p.color};" data-id="${p.id}"><div class="gauge-fill" style="width:${Math.min((p.progress||0)*25,100)}%;background:${p.color};"></div><span>${p.label}</span></div>`;
  return `<div class="picker-item ${p.type === 'routine' ? 'bubble bubble-routine' : 'bubble'}" style="background:${p.color};" data-id="${p.id}">${p.label}</div>`;
}

function validatePicker() {
  pickerSelected.forEach(id => {
    const p = state.praxis.find(x => x.id === id); if (p) p.active = true;
...
function openGaugeSheet(id) {
  const p = state.praxis.find(x => x.id === id); if (!p) return;
  const sheet = getSheet(); let selected = p.progress || 0;
  sheet.innerHTML = `<div class="sheet-handle"></div><div class="sheet-title">${p.label}</div><div class="gauge-sheet-track"><div class="gauge-sheet-fill" id="gaugeSheetFill" style="width:${selected * 25}%; background:${p.color};"></div></div><div class="gauge-sheet-steps">${[0,1,2,3].map(s => `<button class="gauge-step-btn ${selected === s ? 'selected' : ''}" data-step="${s}" style="--gc:${p.color};">${s === 0 ? '0%' : (s * 25) + '%'}</button>`).join('')}</div><div class="sheet-actions" style="margin-top:20px;"><button class="btn-sheet btn-cancel" id="btnGaugeCancel">Annuler</button><button class="btn-sheet btn-create-action" id="btnGaugeSave">Valider</button></div>`;

  const fill = sheet.querySelector('#gaugeSheetFill');
  sheet.querySelectorAll('.gauge-step-btn').forEach(btn => { btn.addEventListener('click', () => { selected = parseInt(btn.dataset.step); sheet.querySelectorAll('.gauge-step-btn').forEach(b => b.classList.remove('selected')); btn.classList.add('selected'); fill.style.width = (selected * 25) + '%'; }); });
  sheet.querySelector('#btnGaugeCancel').addEventListener('click', closeSheet);
  sheet.querySelector('#btnGaugeSave').addEventListener('click', () => { const prevProgress = p.progress || 0; if (selected !== prevProgress) { pushUndo({ type:'accueil_long_progress', id, prevProgress, exploded: false, page:'accueil' }); p.progress = selected; saveState(); } closeSheet(); renderAccueil(); });
  document.getElementById('sheetOverlay').classList.add('open'); sheet.classList.add('open');
}

function openNoteSheet() {
  const sheet = getSheet(); const histHTML = state.noteHistory.length ? `<div class="note-history-row">${state.noteHistory.slice(0,6).map((n,i) => `<div class="note-hist-bubble" data-hidx="${i}">${n.toLowerCase()}</div>`).join('')}</div>` : '';
  sheet.innerHTML = `<div class="sheet-handle"></div><div class="sheet-row"><input class="sheet-input" id="noteInput" type="text" placeholder="Note…" autocapitalize="sentences" spellcheck="true"></div>${histHTML}<div class="sheet-actions"><button class="btn-sheet btn-cancel" id="btnNoteCancel">Annuler</button><button class="btn-sheet btn-create-action" id="btnNoteCreate">Ajouter</button></div>`;

  const input = sheet.querySelector('#noteInput');
  function addNote(label) { if (!label) return; state.notes.push({ label, color: pickNoteColor() }); state.noteHistory = [label, ...state.noteHistory.filter(n => n !== label)].slice(0, 6); saveState(); closeSheet(); renderAccueil(); }
...
function openConfirmSheet(title, sub, confirmLabel, onConfirm) {
  const sheet = getSheet(); sheet.innerHTML = `<div class="sheet-handle"></div><div class="sheet-confirm-text">${title}</div><div class="sheet-confirm-sub">${sub}</div><div class="sheet-actions" style="margin-top:20px;"><button class="btn-sheet btn-cancel" id="btnConfirmCancel">Annuler</button><button class="btn-sheet btn-delete" id="btnConfirmOk">${confirmLabel}</button></div>`;
  sheet.querySelector('#btnConfirmCancel').addEventListener('click', closeSheet); sheet.querySelector('#btnConfirmOk').addEventListener('click', () => { closeSheet(); onConfirm(); });
  document.getElementById('sheetOverlay').classList.add('open'); sheet.classList.add('open');
}
...
function computeStreak(history) {
  ...
  for (let d=0; d<=83; d++) {
    const date = new Date(today); date.setDate(date.getDate()-d); const key = localDateKey(date); const h = history[key];
    if (key === todayStr && (!h || h.done < h.total)) continue; if (!h || h.total===0) continue; if (h.done===h.total) streak++; else break;
  }
  return streak;
}
...
function renderStats() {
  ...
  const praxisStats=routines.map(p => {
    ...
    const rate = pTotal > 0 ? Math.round(pDone/pTotal*100) : 0; const todayStr = localDateKey(today);
    for (let d=0; d<=83; d++) { const date = new Date(today); date.setDate(date.getDate()-d); const key = localDateKey(date); const h = history[key]; if (p.days && !p.days.includes(((date.getDay()+6)%7)+1)) continue; if (key === todayStr && (!h || !(h.ids && h.ids.includes(p.id)))) continue; if (!h || h.total === 0) continue; if (h.ids && h.ids.includes(p.id)) pStreak++; else break; }
...
function openStatsHelp() {
  const sheet=getSheet(); sheet.innerHTML=`<div class="sheet-handle"></div><div class="sheet-title">Comprendre les stats</div><div class="stats-help-content">${[['Jours consécutifs','Nombre de jours d\'affilée où toutes tes routines programmées ont été complétées.'],['Record','Ta meilleure série depuis le début.'],['30 jours','Taux de complétion global sur les 30 derniers jours (routines réalisées / routines programmées).'],['Total','Nombre de journées où toutes tes routines ont été complétées, depuis le début.'],['Historique','Chaque case = un jour. Bleu plein = toutes les routines faites. Bleu atténué = partiellement. Gris = aucune.'],['Par praxis','Série individuelle et taux de complétion sur 30 jours pour chaque routine active.'],['Couronne de laurier','Plus ta série est longue, plus la couronne est dense. Elle reflète ton engagement quotidien.']].map(x=>`<div class="stats-help-item"><div class="stats-help-label">${x[0]}</div><div class="stats-help-desc">${x[1]}</div></div>`).join('')}</div><div class="sheet-actions"><button class="btn-sheet btn-activate" id="btnHelpClose">Fermer</button></div>`;
...
(function() {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    const today = todayKeyStatic(); if (today !== lastDay) { lastDay = today; loadAccueil(); if (state.currentPage === 'accueil') renderAccueil(); }
  });
})();